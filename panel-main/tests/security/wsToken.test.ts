...
): { serverId: string; userId: number } | null {
...
describe("WebSocket token security", () => {
  ...
  it("rejects tokens with missing fields", () => {
    ...
    const p1 = b64url(
      Buffer.from(
        JSON.stringify({ v: VERSION, usr: 42, exp: Date.now() + 60000 }),
      ),
    );
    const s1 = crypto
      .createHmac("sha256", secret())
    ...
    const p2 = b64url(
      Buffer.from(
        JSON.stringify({ v: VERSION, srv: "x", exp: Date.now() + 60000 }),
      ),
    );
    const s2 = crypto
      .createHmac("sha256", secret())
    ...
    // Missing exp
    const p3 = b64url(
      Buffer.from(JSON.stringify({ v: VERSION, srv: "x", usr: 1 })),
    );
    const s3 = crypto
      .createHmac("sha256", secret())
      .update(p3)
