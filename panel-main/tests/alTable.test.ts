...
function matchCompound(el, seg) {
  ...
  const classNames = (rest.match(/\.([a-zA-Z0-9_-]+)/g) || []).map((c) =>
    c.slice(1),
  );
  ...
  constructor(tag, attrs = {}) {
    ...
    if (attrs.class)
      String(attrs.class)
        .split(/\s+/)
        .filter(Boolean)
        .forEach((c) => this._classListSet.add(c));
  }
  get dataset() {
    const out = {};
...
function makeTable({
  attrs = {},
  labels = ["Image", "Version", "Status", "Actions"],
  rows = 2,
} = {}) {
  const table = new FakeEl(
    "table",
    Object.assign({ class: "al-table" }, attrs),
  );
...
describe("al-table responsive fallback", () => {
  ...
  it('skips card fallback when data-table-card="off"', () => {
    const { table, container } = makeTable({
      attrs: { "data-table-card": "off" },
    });
    window.alTableScan(container);
  ...
  it("injects a stable empty-state row when there are no data rows", () => {
    const { table, tbody, container } = makeTable({
      rows: 0,
      attrs: {
        "data-table-empty": "No images yet",
        "data-table-empty-colspan": "4",
      },
    });
    window.alTableScan(container);
  ...
  it("removes the placeholder when a real row is present after a refresh", () => {
    const { tbody, container } = makeTable({
      rows: 0,
      attrs: { "data-table-empty": "No images yet" },
    });
    window.alTableScan(container);
