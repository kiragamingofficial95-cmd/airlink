...
describe("validation boundary (parseBody/parseParams/parseQuery)", () => {
  ...
  beforeEach(async () => {
    ...
    app.post("/body", parseBody(schema), (req, res) => {
  ...
  it("rejects missing core fields with the legacy message", () => {
    ...
    if (!r.success) {
      expect(r.error.issues[0]?.message).toBe(
        "Missing required fields: email, username, or password.",
      );
    }
  });
...
describe("updateUserSchema", () => {
  ...
  it("accepts numeric limit strings and null", () => {
    expect(updateUserSchema.safeParse({ serverLimit: null }).success).toBe(
      true,
    );
    expect(updateUserSchema.safeParse({ maxMemory: "2048" }).success).toBe(
      true,
    );
  });
});
