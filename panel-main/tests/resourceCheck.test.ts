...
describe("assertNodeCapacity", () => {
  ...
  it("respects overallocation for memory", async () => {
    ...
    await expect(assertNodeCapacity(node, 12289, 100, 20480)).rejects.toThrow(
  ...
  it("runningOnly filters to running servers in the query", async () => {
    ...
    await assertNodeCapacity(baseNode, 1024, 100, 20480, undefined, {
      runningOnly: true,
    });
  ...
  it("runningOnly ignores stopped servers (freed resources)", async () => {
    ...
    mockPrisma.server.findMany.mockImplementation(({ where }: any) => {
      ...
      return Promise.resolve([
        { Memory: 4096, Cpu: 100, Storage: 51200, Running: false },
      ]);
    });

    await expect(
      assertNodeCapacity(baseNode, 4096, 100, 20480, undefined, {
        runningOnly: true,
      }),
    ).resolves.toBeUndefined();
  });
  ...
  it("runningOnly counts running servers against capacity", async () => {
    ...
    mockPrisma.server.findMany.mockImplementation(({ where }: any) => {
      if (where?.Running === true)
        return Promise.resolve([
          { Memory: 8193, Cpu: 100, Storage: 51200, Running: true },
        ]);
      return Promise.resolve([
        { Memory: 8193, Cpu: 100, Storage: 51200, Running: true },
      ]);
