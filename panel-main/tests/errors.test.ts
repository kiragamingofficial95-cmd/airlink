...
describe("errors sanitization contract", () => {
  ...
  describe("sanitizeError / safeClientMessage", () => {
    it("never exposes internal details in the safe message", () => {
      const raw = new Error(
        "ECONNREFUSED 127.0.0.1:3306 user=root host=/var/lib/mysql",
      );
      const info = sanitizeError(raw);
      expect(info.safeMessage).not.toMatch(
        /127\.0\.0\.1|3306|root|\/var\/lib\/mysql/,
      );
    ...
    it("classifies daemon markers", () => {
      expect(
        sanitizeError(
          new Error("docker: no such container abc /var/lib/docker"),
        ).category,
      ).toBe("daemon");
    });
    ...
    it("classifies filesystem markers", () => {
      expect(
        sanitizeError(new Error("EACCES: permission denied, open /etc/passwd"))
          .category,
      ).toBe("filesystem");
    });

    it("classifies network markers", () => {
      expect(sanitizeError(new Error("fetch failed: ETIMEDOUT")).category).toBe(
        "network",
      );
    });

    it("falls back to unknown for unrecognized messages", () => {
      expect(sanitizeError(new Error("something innocuous")).category).toBe(
        "unknown",
      );
    });
    ...
    it("prefers the caller fallback over the category message", () => {
      expect(
        safeClientMessage(new Error("ECONNREFUSED"), "Failed to connect."),
  ...
  describe("daemonMessage", () => {
    it("relays a trusted daemon structured error field verbatim", () => {
      expect(daemonMessage({ error: "Invalid name" }, "Fallback")).toBe(
        "Invalid name",
      );
    });

    it("relays the message field too", () => {
      expect(daemonMessage({ message: "No space left" }, "Fallback")).toBe(
        "No space left",
      );
    });
