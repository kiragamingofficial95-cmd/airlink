...
beforeEach(() => {
  ...
  vi.spyOn(dns, "lookup").mockImplementation(((
    hostname: string,
    opts: unknown,
    cb: (err: unknown, addrs?: dns.LookupAddress[] | string) => void,
  ) => {
    const addresses: dns.LookupAddress[] = [{ address: PUBLIC_IP, family: 4 }];
    if (typeof opts === "function")
      (opts as (err: unknown, addrs?: dns.LookupAddress[] | string) => void)(
        null,
        addresses,
      );
...
describe("ssrf: assertSafePublicUrl (DNS-aware)", () => {
  it("blocks literal private hosts before any DNS lookup", async () => {
    expect(await assertSafePublicUrl("https://localhost:8080/x")).toEqual({
      ok: false,
      error: expect.stringContaining("Private/internal"),
    });
    expect(await assertSafePublicUrl("https://127.0.0.1/x")).toEqual({
      ok: false,
      error: expect.stringContaining("Private/internal"),
    });
    expect(await assertSafePublicUrl("https://192.168.1.10/x")).toEqual({
      ok: false,
      error: expect.stringContaining("Private/internal"),
    });
  });
  ...
  it("blocks a hostname that resolves to a private address (DNS rebinding)", async () => {
    ...
    expect(await assertSafePublicUrl("https://example.com/x")).toEqual({
      ok: false,
      error: expect.stringContaining("private/internal address"),
    });
  });

  it("accepts a public hostname resolving to a public address", async () => {
    expect(await assertSafePublicUrl("https://example.com/egg.json")).toEqual({
      ok: true,
    });
  });
  ...
  it("accepts a hostname that fails to resolve", async () => {
    ...
    expect(await assertSafePublicUrl("https://no-such-host.invalid/x")).toEqual(
      { ok: true },
    );
  });

  it("enforces protocol rules", async () => {
    expect(await assertSafePublicUrl("ftp://example.com/x")).toEqual({
      ok: false,
      error: expect.stringContaining("http(s)"),
    });
    expect(await assertSafePublicUrl("http://example.com/x")).toEqual({
      ok: false,
      error: expect.stringContaining("https"),
    });
    expect(
      await assertSafePublicUrl("http://example.com/x", { allowHttp: true }),
    ).toEqual({ ok: true });
  });
});
