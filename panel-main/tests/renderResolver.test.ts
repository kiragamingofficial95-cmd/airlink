...
beforeAll(() => {
  ...
  fs.mkdirSync(path.join(addonsDir, "demo-addon", "views"), {
    recursive: true,
  });
  fs.mkdirSync(path.join(addonsDir, "other-addon", "views"), {
    recursive: true,
  });
  fs.writeFileSync(path.join(viewsDir, "primary.ejs"), "<h1>primary</h1>");
  fs.writeFileSync(
    path.join(addonsDir, "demo-addon", "views", "fallback.ejs"),
    "<h1>addon</h1>",
  );
  fs.writeFileSync(
    path.join(addonsDir, "demo-addon", "views", "nested.ejs"),
    "<h1>nested</h1>",
  );
  fs.writeFileSync(
    path.join(addonsDir, "other-addon", "views", "other.ejs"),
    "<h1>other</h1>",
  );
});
...
describe("installRenderResolver", () => {
  ...
  it("renders nested views from the requested addon slug first", async () => {
    const { html, status } = await renderWithResolver("nested", {
      addonSlug: "demo-addon",
    });
    expect(status).toBe(200);
  ...
  it("scans other addons when the requested slug does not own the view", async () => {
    const { html, status } = await renderWithResolver("other", {
      addonSlug: "demo-addon",
    });
    expect(status).toBe(200);
