...
function buildDialog() {
  const dlg = new FakeDialog("dialog", {
    class: "al-dialog",
    "data-al-dialog": "",
  });
...
describe("al-dialog controller", () => {
  ...
  it("confirm resolves true when the confirm action is clicked", async () => {
    ...
    dialog.confirmBtn.fire("click", {
      target: dialog.confirmBtn,
      preventDefault: () => {},
    });
  ...
  it("confirm resolves false on dismiss and close-button actions", async () => {
    ...
    dialog.dismissBtn.fire("click", {
      target: dialog.dismissBtn,
      preventDefault: () => {},
    });
    await expect(p1).resolves.toBe(false);

    const p2 = ALDialog.confirm({ title: "t", body: "b" });
    dialog.closeBtn.fire("click", {
      target: dialog.closeBtn,
      preventDefault: () => {},
    });
  ...
  it("traps Tab within the dialog", async () => {
    ...
    const focusable = dialog.dlg.querySelectorAll(
      'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])',
    );
    const last = focusable[focusable.length - 1];
    scope.document.activeElement = last;
    dialog.dlg.fire("keydown", {
      key: "Tab",
      shiftKey: false,
      preventDefault: () => {},
    });
