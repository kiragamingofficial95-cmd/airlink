(function () {
  ...
  function enableTestButtons() {
    testButtons.forEach((button) => {
      ...
      const hasPermission =
        selectedApiKeyPermissions.includes(requiredPermission);
  ...
  testButtons.forEach((button) => {
    button.addEventListener("click", async function () {
      ...
      const responseContainer = parentSection.querySelector(
        ".response-container",
      );
      const responseOutput = parentSection.querySelector(
        ...
        if (textArea) {
          try {
            ...
            responseStatus.className =
              "response-status text-xs font-medium text-red-500";
            return;
          }
        }
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          responseStatus.className =
            "response-status text-xs font-medium text-green-500";
        } else {
          responseStatus.textContent = `${response.status} ${response.statusText}`;
          responseStatus.className =
            "response-status text-xs font-medium text-red-500";
